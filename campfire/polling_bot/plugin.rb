  ...
  class PollingBot
    class Plugin
      ...
      def self.load_all(bot)
        ...
        Dir.glob(File.dirname(__FILE__) + "/plugins/models/*.rb").each do |model|
          model.match(/(.*?)\.rb$/) && (require $1)
        end
        Dir.glob(File.dirname(__FILE__) + "/plugins/*.rb").each do |plugin|
          plugin.match(/(.*?)\.rb$/) && (require $1)
        end
